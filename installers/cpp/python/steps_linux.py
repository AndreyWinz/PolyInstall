"""
steps_linux.py — Linux steps for the PolyInstall C++ installer (Python variant)
Downloads GCC from ftp.gnu.org, verifies SHA-512, builds with c,c++ languages.
"""
...
def install_linux(version: str, prefix: str | None) -> None:
    if prefix is None:
        prefix = str(Path.home() / ".local" / "gcc-cpp")

    log(f"Target: GCC {version} (C + C++) -> {prefix}")
    ...
    # Configure — C and C++ enabled
    log(f"Configuring GCC build (languages: c,c++, prefix: {prefix})...")
    ...
    ok(f"GCC {version} (C + C++) installed.")

    # PATH
    bin_dir = os.path.join(prefix, "bin")
    update_path_unix(bin_dir, f"GCC {version} (C++)")
