...
```bash
python3 main.py --version 14.2.0 --prefix ~/.local/gcc      # Linux
python3 main.py --version 18.1.8 --prefix ~/.local/llvm     # Windows
python3 main.py                                              # macOS (no options needed)
```

| Flag | Default (Linux) | Default (Windows) | macOS |
|------|----------------|-------------------|-------|
| `--version` | `14.2.0` (GCC) | `18.1.8` (LLVM) | ignored |
| `--prefix` | `~/.local/gcc` | `%USERPROFILE%\.local\llvm` | ignored |
...
### Linux
1. Checks for `curl`, `tar`, `make`, and an existing C compiler (needed to bootstrap GCC)
...
5. Runs `./configure --enable-languages=c --disable-multilib`, `make`, and `make install`
6. Appends `<prefix>/bin` to PATH in `~/.bashrc`, `~/.zshrc`, `~/.zprofile` as appropriate

### macOS
1. Checks whether Xcode Command Line Tools (and thus `clang`) are already installed
...
4. Adds `<prefix>\bin` to the current user's PATH via the Windows registry (`winreg`)
...
| `steps_linux.py` | Linux-specific steps (GCC build from source) |
| `steps_macos.py` | macOS-specific steps (Xcode CLT) |
| `steps_windows.py` | Windows-specific steps (LLVM/Clang zip) |
