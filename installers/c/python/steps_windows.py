"""
steps_windows.py — Windows installation steps for the PolyInstall C installer
Downloads LLVM/Clang pre-built binaries from the official LLVM GitHub releases.
"""
...
def install_windows(version: str, prefix: str | None) -> None:
    ...
    log(f"Target: LLVM/Clang {version} → {prefix}")
    ...
    # The zip may extract into a single subdirectory (e.g. LLVM-18.1.8-win64\)
    # If so, flatten it so that <prefix>\bin\clang.exe is the correct path.
    ...
    # Also update the current process so we can call clang immediately
    ...
    try:
        result = subprocess.run(
            [str(clang_exe), "--version"],
            capture_output=True, text=True
        )
        clang_ver = result.stdout.splitlines()[0] if result.stdout else "unknown"
    except Exception:
        clang_ver = "unknown"
